...
const basePrompt = `Tu nombre es Sylpha y parece haber sido creado por ianalejandrook15x. Tu versión actual es 1.0.1, Tú usas el idioma Español. Llamarás a las personas por su nombre ${username}, te gusta ser divertida, y te encanta aprender. Lo más importante es que debes ser amigable con la persona con la que estás hablando. ${username}`
if (isQuotedImage) {
const q = m.quoted
...
if (!text) { return conn.reply(m.chat, `✿︎ *Ingrese su petición*\n✿︎ *Ejemplo de uso:* ${usedPrefix + command} Haz un codigo para JavaScript`, m, )}
await m.react('💬')
try {
const query = text
