...
await this.sendMessage(m.chat, { text: `✧ ${usuario} *Ha realizado un cambio de nombre en el grupo*`, mentions: [m.sender], mentions: (await conn.groupMetadata(m.chat)).participants.map(v => v.id) }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100}) 

} else if (chat.detect && m.messageStubType == 22) {
await this.sendMessage(m.chat, { text: `✧ ${usuario} *Ha realizado un cambio en imágen del grupo*`, mentions: [m.sender] }, { quoted: fliveLoc, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100}) 
...
await this.sendMessage(m.chat, { text: `✧ *Ahora ${m.messageStubParameters[0] == 'on' ? 'solo administradores' : 'todos'} podran editar la información del grupo*`, mentions: [m.sender] }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
...
let txt2 = `✧ *Un administrador menos*\n\n`
